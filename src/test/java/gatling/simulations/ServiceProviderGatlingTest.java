 ...
 */
public class ServiceProviderGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all serviceProviders").get("/api/service-providers").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new serviceProvider")
                ...
                .repeat(5)
                .on(
                    exec(http("Get created serviceProvider").get("${new_serviceProvider_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created serviceProvider").delete("${new_serviceProvider_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ServiceProvider entity").exec(scn);
